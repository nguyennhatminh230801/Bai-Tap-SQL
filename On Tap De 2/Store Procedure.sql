--Bài nào liên quan đến Thủ Tục thì sài Procedure
--Bài nào liên quan đến Hàm thì function
--Bài nào liên quan đến khung nhìn thì view

--Kiểu 1 Procedure Không đầu ra
--create proc TenThuTuc(@tb1 kieudl1, @tb2 kieudl2, ...)
--as
	--begin
		
	--end

--exec TenThuTuc N'asdasd', N'xzysad'
--Procedure giống function nhưng cái xử lý thì không thiên về output(Tí nữa có thủ tục output)

--Kiểu 2: Procedure Có đầu ra
--create proc TenThuTuc(@tb1 kieudl1, @tb2 kieudl2, ..., @kq int output)
--as
	--begin
...
--declare @Ketqua int
--exec TenThuTuc N'asdasd', N'asdasdas', ..., @Ketqua output
--select @Ketqua as N'Kết quả'
...
as
	begin
		if(exists(select * from Khoa where Makhoa = @Makhoa))
			begin
				print 'Khoa đã tồn tại trong cơ sở dữ liệu'
			end
		else
			begin
